      ...
      if (result.isConfirmed) {
        submitForm(body).then(response => {
          ...
          if(finalGrade == 10){
            Swal.fire({
              title: "Parabéns!",
              text: "Você acertou tudo! Se essa foi sua primeira tentativa, vá reivindicar seu prêmio!",
              icon: "success"
            }).then((result) => {
              if (result.isConfirmed) {
            ...
            Swal.fire({
              title: "NA TRAAAAVE!!",
              text: "Você acertou QUASE tudo! Se essa foi sua primeira tentativa, você merece uma lembrancinha pela desempenho",
              icon: "success"
            }).then((result) => {
              if (result.isConfirmed) {
                window.location.reload();
            ...
            Swal.fire({
              title: "Nada mal!",
              text: "Pelo nível de dificuldade do quiz você mandou muito bem!",
              icon: "info"
            }).then((result) => {
              if (result.isConfirmed) {
                window.location.reload();
            ...
            Swal.fire({
              title: "Errrrroooou!",
              text: "Realmente tava bem difícil né? Que tal aproveitar a oportunidade para me conhecer um pouco melhor?",
              icon: "question"
            }).then((result) => {
              if (result.isConfirmed) {
                window.location.reload();
